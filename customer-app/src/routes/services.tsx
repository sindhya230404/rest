...
import { Droplet, Sparkles, Trash2, Receipt, ConciergeBell, CheckCircle2 } from "lucide-react";
...
function Services() {
  ...
  const handleRequest = async (id: string, label: string) => {
    setLoading(true);
    try {
      // Broadcast live service request to Kitchen / KDS
      liveOrderStore.addServiceRequest({
  ...
  const combinedRequests = [
    ...liveServices.filter((s) => s.table_number.toLowerCase() === tableNumber.toLowerCase()),
    ...requests.filter((r) => !liveServices.some((ls) => ls.id === r.id)),
  ];
    ...
    <div className="min-h-screen bg-background pb-32">
      ...
      <div className="max-w-3xl mx-auto px-4 md:px-8 pt-6">
        ...
        <div className="mt-6 grid gap-3 sm:grid-cols-2">
          {services.map((s) => {
            const activeReq = combinedRequests.find((r) => r.service_type === s.id && r.status !== "completed");
            return (
              <motion.button
                key={s.id}
                whileTap={{ scale: 0.97 }}
