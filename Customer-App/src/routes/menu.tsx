...
import { Search, SlidersHorizontal, Utensils, Clock, CheckCircle2, ChevronRight, Flame } from "lucide-react";
...
import { useLiveOrders, type LiveOrder } from "@/lib/live-order-store";
...
function Menu() {
  ...
  const activeTableOrders = useMemo(() => {
    const norm = tableNumber.toLowerCase().replace(/\s+/g, "");
    return allLiveOrders.filter((o) => {
      const oTable = o.table.toLowerCase().replace(/\s+/g, "");
      return (oTable === norm || oTable.includes(norm) || norm.includes(oTable)) && o.status !== "completed";
    });
          ...
          <div className="flex gap-2 items-center">
            <div className="relative flex-1 md:w-72">
              ...
              <Input
                ...
                className="pl-9 rounded-full bg-card border"
              />
            </div>
            <button
              onClick={() => setVegOnly((v) => !v)}
              className={`shrink-0 h-10 rounded-full px-4 text-xs font-semibold border flex items-center gap-2 ${
                vegOnly ? "gradient-accent text-white border-transparent" : "bg-card"
              }`}
            >
          ...
          {categories.map((c) => {
            ...
            return (
              <button
                key={c.id}
                onClick={() => setCat(c.id)}
                className={`relative shrink-0 rounded-2xl border px-4 py-3 min-w-[92px] text-sm font-semibold flex flex-col items-center gap-1 ${
          ...
          {combos.map((c) => (
            <motion.div key={c.id} whileHover={{ y: -3 }} className="relative overflow-hidden rounded-3xl shadow-float">
              ...
              <div className="absolute inset-0 p-4 flex flex-col justify-end text-white">
                <div className="text-[10px] font-bold uppercase tracking-widest bg-white/20 rounded-full px-2 py-0.5 w-fit">
                  Save ₹{c.save}
                </div>
