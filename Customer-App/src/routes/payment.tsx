...
function Payment() {
  ...
  const pay = async () => {
    ...
    const customerInfoFormatted = `${custName.trim()} (Ph: ${custPhone.trim()})`;
    ...
    if (sel === "cash") {
      ...
      setState("success");
      toast.success(`Order confirmed for ${custName.trim()}! Cash payment at counter.`);
      return;
    }
    ...
    const options = {
      ...
      handler: async function (response: any) {
        ...
        setState("success");
        toast.success(`Payment Successful for ${custName.trim()}! Transaction ID: ${paymentId}`);
      },
      prefill: {
        name: custName.trim(),
        email: custEmail.trim() || "customer@emberandoak.com",
