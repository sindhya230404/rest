...
import { Dialog, DialogContent, DialogHeader, DialogTitle, DialogTrigger, DialogFooter } from "@/admin/components/ui/dialog";
import { Receipt, Search, Download, Printer, Split, Percent } from "lucide-react";
...
function InvoicesPage() {
  ...
  return (
    <div>
            ...
            <TableBody>
              {filtered.map((inv) => (
                <TableRow key={inv.id} className="hover:bg-muted/40">
                  ...
                  <TableCell>
                    <Button variant="outline" size="sm" onClick={() => setSelectedInvoice(inv as Invoice)}>
                      Preview
                    </Button>
                  </TableCell>
                </TableRow>
      ...
      {/* Centered Modal / Popup for Invoice Preview */}
      <Dialog open={!!selectedInvoice} onOpenChange={(open) => !open && setSelectedInvoice(null)}>
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            <DialogTitle>Invoice Preview — {selectedInvoice?.invoice || selectedInvoice?.id}</DialogTitle>
          </DialogHeader>
          {selectedInvoice && (
            <div>
              <InvoicePreview inv={selectedInvoice} />
  ...
  return (
    <Card className="mt-2 p-6 shadow-xs border">
      <div className="text-center">
        <div className="font-display text-2xl font-bold">ScanDine</div>
