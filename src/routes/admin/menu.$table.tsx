...
function CustomerMenu() {
  ...
  const { data: dbMenuItems } = useSupabaseTable<MenuItem>("menu_items");
  ...
  useEffect(() => {
    const handleStatusChange = (e: Event) => {
      ...
      if (customEv.detail) {
        const { orderId, tableNumber: evTable, status } = customEv.detail;
        if (evTable === tableNumber || orderId === placedOrderId) {
          setCurrentOrderStatus(status);
          if (status === "ready") {
            toast.success("🚀 Your order is Out for Delivery! It will arrive at your table shortly.", { duration: 8000 });
          } else if (status === "preparing") {
            toast.info("👨‍🍳 Kitchen has started preparing your order!");
  ...
  // Use DB items if available, otherwise fall back to mock
  const allItems: DisplayItem[] = dbMenuItems.length > 0
    ? dbMenuItems.map((m) => ({
        id: m.id,
        name: m.name,
        ...
        // Defaults for fields that exist only in mock
        spicy: 0,
        veg: false,
        popular: false,
        featured: false,
          ...
          <SheetContent side="bottom" className="max-h-[90vh] rounded-t-3xl">
            ...
            <div className="mt-4 space-y-2 border-t pt-4 text-sm">
              ...
              <div className="mt-3 space-y-1">
                <div className="flex justify-between"><span className="text-muted-foreground">Subtotal</span><span>{restaurantInfo.currency}{total.toFixed(2)}</span></div>
                <div className="flex justify-between"><span className="text-muted-foreground">GST (8%)</span><span>{restaurantInfo.currency}{(total * 0.08).toFixed(2)}</span></div>
                <div className="flex justify-between font-display text-lg font-bold"><span>Total</span><span className="text-primary">{restaurantInfo.currency}{(total * 1.08).toFixed(2)}</span></div>
              </div>
            ...
            <SheetFooter className="mt-4">
              ...
              >
                {placingOrder ? "Placing order…" : `Place order · ${restaurantInfo.currency}${(total * 1.08).toFixed(2)}`}
              </Button>
            </SheetFooter>
          </SheetContent>
        </Sheet>
      ...
      <Dialog open={placed} onOpenChange={setPlaced}>
        <DialogContent className="max-w-sm rounded-3xl">
          ...
          <div className="text-center text-sm text-muted-foreground">
            Your order <span className="font-semibold text-foreground">#{placedOrderId}</span> is now with the kitchen. Estimated 14 minutes.
          </div>
          <div className="rounded-2xl bg-muted/50 p-4">
            <div className="flex items-center justify-between text-xs">
              <span className="text-muted-foreground">Table</span><span className="font-semibold">{table}</span>
            </div>
            <div className="mt-2 flex items-center justify-between text-xs">
              <span className="text-muted-foreground">Status</span>
              <span className="rounded-full bg-warning/15 px-2 py-0.5 text-warning">Pending</span>
            </div>
          </div>
          <div className="flex gap-2">
