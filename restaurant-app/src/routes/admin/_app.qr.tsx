...
import {
  QrCode, ExternalLink, Printer, Sparkles, Check, Copy, RotateCcw, Trash2
} from "lucide-react";
...
export const Route = createFileRoute("/admin/_app/qr")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Generate, popup, print and manage live table-specific QR codes." },
    ],
  }),
...
const DEFAULT_PRESET_TABLES: StoredTableQr[] = Array.from({ length: 6 }, (_, i) => ({
  id: `table-${i + 1}`,
  name: `Table ${i + 1}`,
  url: `http://192.168.1.15:8080/?table=${i + 1}`,
  createdAt: new Date().toISOString(),
}));

function AdminQrPageRedesigned() {
  const [tableInput, setTableInput] = useState("");
  const [customerAppUrl, setCustomerAppUrl] = useState("http://192.168.1.15:8080");
  ...
  useEffect(() => {
    if (typeof window !== "undefined") {
      try {
        ...
        const detectedUrl = (host && host !== "localhost" && host !== "127.0.0.1")
          ? `${currentProtocol}//${host}:8080`
          : "http://192.168.1.15:8080";

        setCustomerAppUrl(detectedUrl);
  ...
  const getTableUrl = (rawName: string) => {
    const cleanHost = customerAppUrl.replace(/\/$/, "");
    const cleanName = rawName.trim().toLowerCase().startsWith("table")
      ? rawName.trim()
      : `Table ${rawName.trim()}`;
    return `${cleanHost}/?table=${encodeURIComponent(cleanName)}`;
  };
    ...
    <div>
      <div className="print:hidden">
        <PageHeader
          ...
          description="Enter table number to instantly generate a live QR code for customer ordering."
        ...
        <Card className="mb-8 p-6 shadow-md border-amber-500/20">
          ...
          <p className="text-xs text-muted-foreground mb-4">
            Type the table number (e.g. <code>1</code>, <code>2</code>, <code>12</code>) or custom area name (e.g. <code>VIP-1</code>, <code>Terrace-3</code>).
          </p>
